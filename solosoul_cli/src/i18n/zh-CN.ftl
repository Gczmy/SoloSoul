...
### App
app-field-password-verify = 字段 [{$field}] 为 {$level} 级别，请输入主密码验证
app-password-verify-failed = 主密码验证失败
app-password-verify-error = 验证失败：{$err}
app-session-timeout-locked = 会话已超时锁定
app-biometric-not-enabled = 当前账户未启用生物识别登录
app-account-name-empty = 账户名不能为空
app-password-too-short = 主密码至少需要 8 位
app-password-mismatch = 两次输入的密码不一致，请重新设置
app-exit-wizard-confirm = 退出创建账户？未保存的数据将不会被保留。
app-exit-cli-confirm = 退出 SoloSoul CLI？
app-account-create-failed = 创建账户失败：{$err}
app-login-failed = 登录失败：{$err}
app-biometric-unlock-failed = 生物识别解锁失败：{$err}
app-unknown-command = 未知命令：{$cmd}
app-not-in-wizard = 当前不在向导中
app-save-failed = 保存失败：{$err}
app-no-changes-to-save = 当前没有可保存的更改
app-object-name = 对象名称
app-vault-locked = Vault 未解锁
app-unlock-vault-title = 解锁 SoloSoul Vault
app-editing-preferences = 正在编辑自定义偏好...
app-error-overlay = ! 错误
app-info-overlay = ℹ 信息
app-esc-to-close = 按 Esc 关闭

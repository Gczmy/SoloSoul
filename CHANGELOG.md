...
- **同步冲突对话框布局重构：恢复常规滚动，列表不固定** — 根因：关闭「只看差异」时 diff 表格显示全部字段行，内容超长把顶部冲突列表挤出 Dialog 视口（用户开启开关后内容骤减、滚动位置回弹到顶部，才看到列表，产生「开启能看到卡片、关闭看不到」的错觉）。第一版修复（固定高度容器 + 详情区独立滚动）被用户反馈「表格看不到、没有外部滚动条」——内部嵌套滚动区在 macOS 遮罩滚动条下不可感知；第二版改为列表 `position: sticky` 固定顶部，用户进一步反馈「改成和其他一样不 sticky」。最终方案：容器自然高度、列表不固定，列表与详情随 Dialog 整窗滚动（外部滚动条统一控制），与其它对话框交互一致。
- **同步冲突对话框新增位置导航** — 详情头部显示「冲突 X / N」（当前查看第几条、共 N 条）+ 上一条/下一条切换按钮，用户无需滚动顶部列表即可逐个查看全部冲突，明确感知 6 条冲突逐一可查（配合列表计数提示与高度放宽）。
- **同步冲突对话框列表不再被裁切** — 冲突列表容器高度由固定 160px 放宽至 `min(40vh, 320px)`（超出仍可滚动），修复 6 条冲突只显示 1-2 条、看起来「只有一个」的问题；列表顶部新增「共 N 条冲突」计数提示，明确可滚动查看全部。
- **同步冲突 diff 新增「只看差异」切换** — 冲突详情顶栏开关（ToggleSwitch）开启后隐藏所有无差异字段行与叶子条目，仅保留有差异内容（含单侧缺失），并显示「共 N 处差异」计数；对象/数组字段的 `__fields` 摘要等无差异行同步隐藏。开启后无差异时显示「无差异字段」空态（与「无可比字段」区分）。

### Changed

- **同步冲突对话框底部操作按钮悬浮固定** — 「保留本地/保留远程/忽略」按钮栏改为 `position: sticky`：Dialog 内滚动时始终悬浮可见，用户无需滚到底即可快速处理冲突；`bottom: -24px` 让背景条在悬浮时延伸到卡片最底边（覆盖 Dialog 底部 24px padding 区），滚动内容无法从按钮栏下方缝隙漏出；左右负外边距铺满宽度、实底背景 + 顶边框遮挡经过内容，滚动到底自然到位不遮挡末行（浏览器实测：悬浮时按钮栏底边与卡片底边 diff=0）。
...
- **冲突对话框表格不可见修复** — 上一版固定高度布局（`.container` `height: min(70vh,560px)` + `overflow: hidden`，详情区 `flex:1` 独立滚动）在用户实际环境出现「列表可见但表格看不到、无外部滚动条」：嵌套滚动区 + 遮罩滚动条导致详情无法感知滚动。经两版迭代（列表 sticky 固定 → 用户要求不 sticky）最终改为：容器自然高度 + 列表不固定 + Dialog 整窗外部滚动（见上方 Changed），消除嵌套滚动区并恢复外部滚动条。

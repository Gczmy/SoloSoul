...
- **附件批量操作** — 附件管理支持批量选择、批量删除与恢复，支持附件重命名。
- **插件自适应 Key 截断** — 结果区 Key 根据内容自适应截断，避免长 Key 过早换行。
- **插件国家徽章本地化** — 地址格式化器国家显示支持中英文本地化。
- **侧边栏插件结果 UI 统一** — QuickRunningInfo 与插件页面共享渲染组件。
- **OCR MRZ 模板匹配 Strategy C** — 提升 MRZ 识别准确率。

### Fixed

- **附件/快照徽章溢出** — 数量超过 99 时显示 99+。
- **插件 UI 修复集** — 徽章布局、Key/Value 截断、侧边栏对齐、日志展开、Toast 等 10+ 项问题。
- **ESLint / Clippy** — 修复 E001-E007 ESLint 及 11 个 clippy 警告。

...
- **Android 更新下载通知** — 使用系统进度通知显示已下载大小与总大小，完成后可返回应用安装；未授予通知权限时仍可在应用内下载。
- **插件真实安装进度与取消** — 侧栏插件卡片与完整插件页面共用进度圆环和中心取消按钮；下载阶段按真实字节数推进，校验、写入与收尾阶段计入整体进度，成功完成后才显示 100% 和完成标记。未知下载总大小时停留在已确认阶段，不按时间伪造百分比；取消会停止原生下载并防止重复安装。
- **已知设备自定义名称** — 设备详情支持编辑当前账户中的设备备注，留空恢复默认名称；备注保存后同步更新列表与详情，重新发现或配对不会覆盖用户名称。
- **成功更新源记忆与日志** — 记住成功读取版本信息的清单或 Release 来源，下次优先尝试；失败、超时或偏好过期后探测其他允许来源。新增成功日志，记录来源主机、当前版本、读取版本及是否发现更新。

### Changed

- **版本信息获取** — Android 增加静态 `release.json` 及代理入口，与 GitHub API 错峰请求；启动先恢复缓存提示，联网或回到前台时重试。关于页本机版本不再等待在线检查。
- **Android 对象与导航** — 对象卡片显示页面与模板名称，使用图标徽章列出全部字段敏感等级；全部对象和预定义页面统一返回按钮，设备详情操作避免文字换行。
- **Windows 材质层级** — 保留系统 Mica 导航与标题栏，AppBar、通知区和正文共用实色表面；移除侧栏、内容和卡片的重复描边，使用间距、轻阴影和圆角区分层级，支持四种导航位置及深浅主题。紧凑菜单采用 WebView 内局部模糊，正文面板保持实色，并遵循减少透明度、高对比和减少动态效果设置。
- **桌面按钮与危险操作** — macOS 顶栏新增、模板等操作采用一致的平面样式，指南保持最右侧；危险操作保留红色文字及淡红悬停反馈。附件回收站悬停底色与红色边框匹配，已知设备及详情的忘记按钮常态保留淡红底色；Windows 主次按钮、工具栏及指南同步减少多余边框。
- **设备名称编辑布局** — 桌面设备详情限制名称输入框宽度，取消与保存放在输入框同一行右侧，提示文字留在下方；窄屏按可用空间换行，保留移动端触控范围。
- **插件固定排序** — 侧栏插件卡片与完整页面按当前显示名称排序，中文按拼音、英文按字母并识别数字，重名时以插件 ID 稳定排序；重新打开或刷新不再随后端集合遍历顺序变化。
- **统一应用图标** — 应用内品牌图标复用安装包图标；关于页保留完整图标和原有底色，仅修正四角白边与阴影轮廓。

### Fixed

- **跨页面更新状态** — 登录、解锁、横幅与关于页共用一个更新任务，页面卸载不再中断下载或恢复重复下载入口；忽略旧请求迟到的进度。
- **下载进度与重启恢复** — 显示当前文件绝对字节数，换源不累计重复流量；读取持久断点并原子保存元数据，旧清单不得清除新版本断点入口。服务器拒绝续传或缓存失效时仍重新下载。
- **更新说明表格仍显示源码** — 移除横幅中动态加载失败后静默显示原始 Markdown 的分支，三个入口直接共用 GFM 渲染器；修正窄屏弹窗宽度及表格滚动容器，并补充生产构建回归，长文件名与宽表格不再撑开弹窗。
- **敏感字段复制顺序** — 遮罩中的敏感字段先揭示再复制；关键字段先验证再复制，验证取消、切换对象或迟到的认证结果不会触发复制，字段组与子字段使用同一流程。
- **Android 对象详情返回** — 系统返回先关闭对象详情，保留当前对象页面、分类与搜索条件；历史记录、附件、密码验证及嵌套照片查看器按层关闭，避免一次返回直接进入首页或连带关闭父卡片。
- **插件卸载确认** — 修复侧栏确认弹窗通过 Portal 展示时被误判为点击外部，导致侧栏先卸载、确认后插件仍显示已安装的问题。
- **插件安装入口与反馈** — 侧栏与完整页面统一展示安装、更新、取消及失败状态，关闭再打开卡片仍保留当前进度；补齐 `create_plugin_install` 的原生 IPC 分发，修复点击安装无反馈和 `Command create_plugin_install not found` 错误。
- **照片预览工具栏** — 固定深色缩放栏使用浅色文字，避免页面主题导致百分比等文本看不见；Windows 文件与照片预览顶栏沿用内容 AppBar 表面，不再叠成不匹配的深色背景。

### Security

- **账户设备偏好加密** — 自动同步、设置偏好同步开关与设备备注保存到当前账户的加密 Profile；成功迁移后清理本机旧明文同步开关。解锁和切换账户恢复各自设置，本机同步策略不会被其他设备覆盖，设备备注作为账户数据参与同步。
- **更新源偏好边界** — 登录后的成功源偏好加密保存到账户，登录前只使用不含账户标识的本机来源缓存；偏好仅调整已有允许来源的顺序，不能注入新地址，且不改变更新包签名校验要求。网络请求绑定原账户，切换账户后的结果不写入新账户。
- **安装与更新异步隔离** — 取消、锁定或开始新操作后忽略旧插件进度与完成事件；更新缓存只用于提示与断点恢复，不授权安装，下载仍须重新核对可信元数据与签名。

### Chore

- 补充真实插件下载进度、取消及安装命令分发、插件排序、Android 分层返回、设备偏好迁移与同步隔离、更新源选择、生产版 Markdown 表格、预览顶栏和桌面按钮回归；更新 Windows 材质与更新分发文档。
- 版本号同步为 2.13.2，Android versionCode 为 2013002。

## [2.13.1] - 2026-09-18

### Added

- **更新下载取消** — 桌面与 Android 的全局更新横幅、关于页面及强制更新界面提供取消下载操作；显示取消中状态，停止网络请求与缓存写入，取消后可重新开始下载，已保存且校验身份一致的断点可继续使用。
- **下载线路与速度状态** — 更新界面统一显示正在选择线路、正在切换线路、当前来源主机名和传输速度，配合已有下载字节数与进度；前端只展示主机名，不显示完整下载 URL 或重定向参数。
- **自有更新源配置支持** — 新增编译期 `update-sources.json`，支持自有桌面清单、下载根路径及 Android `release.json`，同时保留 GitHub 与已允许代理的后备路径。默认配置为空，尚未接入实际自有域名或 CDN。

### Changed

- **更新与 OCR 通知布局** — 更新横幅和 OCR 首装提示纳入登录页、创建账户页与主界面的正常布局，通知区域为内容留出空间；统一紧凑样式、窄窗口换行与移动端触控范围，避让原生顶栏、导航与表单。
- **更新传输统一** — 桌面与 Android 共用有界测速、换源和持久断点续传；健康直连优先，实测慢或失败后并发探测备选，按样本吞吐选择线路，下载中持续低速、停流或连接中断时自动尝试其他来源。
- **续传与失败恢复** — 断点关联版本、平台与可信签名或已验签校验和，并记录来源和 ETag；取消、重试或重启后可复用匹配的前缀。源忽略 Range 时从零覆盖，范围或长度异常拒绝续写；没有更快线路时允许可用慢速来源继续完成。

### Fixed

- **更新说明 Markdown 表格** — 全局横幅弹窗、关于页更新卡片和强制更新界面共用 GFM 渲染，下载文件表格不再显示为原始竖线文本；宽表格仅在自身区域横向滚动，长文件名不再撑宽弹窗或手机页面。
- **通知遮挡与认证页滚动** — 修复固定更新/OCR 横幅覆盖顶栏和正文、多个通知挤压操作入口的问题；登录与创建账户布局可随通知高度及 Android 软键盘视觉视口调整，保持表单可滚动访问。
...
- **更换下载线路仍严格验签** — 桌面沿用 updater 原公钥与原产物签名，只有验签通过的下载资源可交给安装命令；Android 在 Rust 侧重新获取并验证校验和签名，拒绝缺少可信校验和的下载，完成后核对 APK SHA-256。
- **传输边界校验** — 更新源与重定向限制为无凭据 HTTPS，下载范围、总大小、响应编码和断点身份均校验；验签或校验和失败清理不可信断点，保留既有有效安装包。更新说明继续禁用原始 HTML 并过滤不安全链接。

### Chore

- **发布分发支持** — 清单生成器新增 `--download-base-url`，可将主清单及全部 legacy 镜像清单的包地址指向同一自有源而保留原签名；四份固定兼容清单始终生成，`--no-probe` 离线运行也不遗漏。
- **Android 元数据与发布验收** — 生成包含 APK、校验和、签名 URL 和字节数的 `release.json`；新增只读分发验收脚本，以有界 Range 读取检查真实资产、Content-Range、大小、首段及远端清单一致性，补充自有源接入、发布顺序和旧客户端兼容边界文档。
- 补充更新说明渲染、取消状态机、通知布局、线路状态、测速换源、断点恢复及发布脚本离线回归；验证流程不替代原发布签名自检和客户端完整性校验。

## [2.13.0] - 2026-09-17

### Added

- **桌面原生材质** — macOS 接入 Liquid Glass，并按系统能力回退到传统毛玻璃等材质；Windows 接入 Mica，保留原生窗口缩放、最大化和贴靠交互，深浅主题同步到原生窗口。
- **Android Material 3 Expressive** — 新增移动端首页、AppBar、悬浮底部导航、工具页、对象列表与底部操作面板，统一主题配色、圆角、间距和选中反馈；平板使用导航轨道。
- **Android 玻璃材质档位** — 提供关闭、局部玻璃（默认）、增强玻璃三档。局部玻璃用于顶栏、底栏和操作弹层；增强模式增加首页液态装饰及设备支持时的原生模糊快捷菜单，能力不足时自动回退。
- **对象尺标导航** — 对当前分类或搜索结果中的全部对象生成等长刻度，悬停显示对象简介，点击滚动定位；当前位置独立高亮，滚动时不再出现半条刻度高亮。
- **云盘目录快捷导出** — 导出页可选择本机检测到的 OneDrive、Dropbox、Google Drive、百度网盘、坚果云等同步目录，并补充阿里云盘、夸克、迅雷、115 常见目录；落盘白名单同步支持这些目标，完成后提示等待云盘客户端同步。
- **启动层与恢复入口** — React 加载前展示品牌启动层，复用缓存外观绘制首帧；初始化超时提供恢复操作，减少空白等待。

### Changed

- **macOS 材质分工** — 原生侧栏、窗口顶栏与工具菜单保留玻璃，紧凑操作菜单使用轻材质；对象详情、历史记录、模板、表单、指南、搜索和 AI 正文使用稳定实色。普通按钮改为平面主次操作与 8px 圆角，保留悬停、按压、键盘焦点及危险操作语义。
- **桌面内容与顶栏布局** — 右侧内容铺满可用区域并延伸至顶栏；统一交通灯区域、AppBar 与拖拽区高度，折叠侧栏保留交通灯所需宽度。Windows 操作栏与内容底色一致，窄窗次要操作收纳到更多菜单。
- **侧栏折叠与快捷卡片** — 折叠按钮移至 SoloSoul 图标右侧，折叠后展开入口置顶；保持图标尺寸与位置，统一添加页面按钮的文字、悬停反馈和卡片风格，弹层按窗口可用高度展开。
- **侧栏工具菜单** — 展开与折叠状态共用悬停展开菜单，减少狭窄滚动区；菜单自下而上展开、自上而下收起，采用更舒缓的动画并遵循减少动态效果偏好，按钮位置与大小保持稳定。
- **Android 新建对象归属** — 从首页全局新建时先选择系统页面或自定义页面，再展示对应模板；从具体页面新建保留当前归属，保存与返回不再绕回归属选择页。
- **Android 页面与操作整理** — 设置首页移除重复个人资料项；页面属性、附件更多操作采用底部面板，对象详情区分主操作与更多操作；从工具页进入功能后返回工具页，模板详情图标与名称同行。
- **Android 触控与附件布局** — 扩大常规按钮、菜单、开关及密码相关操作的触控范围，主要按钮目标至少 48px；全局附件勾选框垂直对齐，附件图标移至名称下方的元信息旁。
- **对象与历史记录阅读** — 对象详情动态字段组采用树状展示，与历史快照结构一致；揭示倒计时追加在既有字段信息之后，不再挤动字段名称或造成值含义误解。
- **辅助功能与资源使用** — 材质尊重减少透明度、高对比和减少动态效果；Android 液态装饰按需绘制，离屏及后台停止绘制，WebGL 不可用时使用静态装饰。

### Security

- **会话与异步请求隔离（P039）** — 锁定、退出或切换账户后废弃旧请求、失败回滚、后续刷新和插件运行事件；列表读取增加乱序保护，防止旧账户数据重新填回已清空缓存或覆盖新账户状态。
- **同步冲突加密落盘（P040）** — 冲突双方的数据纳入 Vault 应用层加密，迁移已有明文冲突记录，并覆盖换钥与密钥探测流程。
- **插件与账户目录边界（P038/P043）** — 拒绝点路径、路径分隔符及其他非法目录 ID；插件读写和卸载共用校验，账户恢复、创建、删除、配置与解锁在产生副作用前校验，防止访问或清理越过目标目录。
- **插件字段授权（P042）** — 单字段与对象列表读取统一按声明、契约、角色及模板绑定投影，未授权名称、属性和嵌套同级字段不再随对象一起返回；保留仅声明角色的插件兼容性。
- **插件会话撤销（P048）** — 缓存命中、宿主调用及返回边界复核 Vault 状态与会话有效期，锁定或过期后清缓存、拒绝继续访问并抑制失效结果。
- **受保护字段展示** — 搜索命中值、同步冲突、回收站详情及快照统一使用掩码与揭示交互；回收站关键字段增加密码验证，复制和展开同样使用受保护值，揭示到期自动隐藏。
- **最近搜索隔离** — 搜索词按账户保存，锁定和退出统一清理，修复读写调用遗漏账户 ID 导致的最近搜索失效。
- **附件临时明文保护（含 P047）** — 导入导出与插件附件使用随机私有临时目录，Unix 目录/文件权限为 0700/0600；一次性目录用后清理，插件 worker 成功、错误和异常退出均释放临时目录，附件解密失败不覆盖原目标或残留半成品。
- **密钥自动擦除（P045）** — 数据密钥实现实际析构清零，移除 Vault 配置中长期保留的密钥副本，打开失败时同样释放并清零。
- **签名脚本保密（P037）** — 读取敏感变量前关闭 shell trace，详细模式仅传递给构建工具；签名私钥经环境变量传递，不再出现在 signer 命令行参数或脚本追踪日志中。
- **开发版 WebDAV 凭证保护** — 拒绝向公网 HTTP 地址发送 Basic 凭证，保留回环和私网调试支持；非法 URL 返回错误而非触发 panic。

### Fixed

- **macOS 窗口恢复** — 修复从 Dock 或台前调度返回时玻璃背景闪黑、交通灯顶栏变形及圆角丢失；分离窗口几何与材质刷新，移除不透明兼容模式，登录卡片底部保留间距。
- **桌面导航稳定性** — 修复返回首页时整窗闪烁、快捷卡片首次定位错误或被内容区遮挡、搜索卡片触发侧栏意外收起，以及折叠时图标缩放、添加页面按钮和弹层跳动；设备卡片悬停不再位移。
- **macOS 文件与照片预览** — 文件预览、照片集和照片查看器避让原生顶栏，解决按钮与交通灯重叠；整条顶栏延续首页原生玻璃效果，不再被预览遮罩涂黑。
- **恢复二维码导出** — 修复内部临时恢复包被 Desktop/Documents/Downloads 导出白名单拒绝，使用专用临时导出路径并自动清理。
- **Android 玻璃可读性** — 首页玻璃与正文共用应用已解析主题，修复增强模式下“我的资料库”文字和数量不可辨认；工具页搜索和模板管理使用与其他入口一致的中性底色。
- **聊天换钥完整性（P041）** — 主密码变更与 KDF 升级将聊天记录纳入同一重加密事务，补全仅有聊天或冲突数据时的密钥探测；旧版本已因换钥损坏的记录仍需旧密码与旧盐值另行恢复。
- **旧页面迁移续传（P046）** — 使用稳定页面 ID 按差集迁移，部分失败保留独立重试来源，重启后可补齐；保留页面元信息与排序，已删除旧页面不重新变为活跃页面。
- **移动端分层返回（P049）** — 浮层历史使用可序列化稳定 ID，修复浏览器克隆 History 状态后将活跃相册误判为残留、返回时额外退出一层的问题。
- **偏好与目录状态** — 补登记 UI 偏好读取命令和云盘相关 ACL；切换保险库目录后发出锁定事件，前后端状态保持一致；修复迟到事件监听与目录进度计时器未清理。
- **启动和跨平台构建** — 修复生产包 Markdown 分包循环初始化导致的启动失败、云同步设置页样式导致的渲染/构建错误，以及 Windows 深色登录背景和原生标题栏主题不一致。
- **恢复数据事务** — 恢复对象修复与快照复制采用完整事务，失败时不留下部分更新；修复失败标记保留，便于后续重试。

### Performance

- **导入预览内存** — 复用流式解密，减少整份明文重复缓冲，降低大包预览的峰值内存。
- **照片集与目录导入** — 照片集首批渲染 200 项并支持加载更多；本地目录导入限制为每批 4 个任务，避免一次并发全部文件。
- **对象与快照存储** — 对象属性采用惰性克隆，快照在同密钥下直接复制密文，减少重复 JSON 分配及解密重加密。
- **重复工作清理** — 合并入站同步后的刷新逻辑，移除重复状态加载与 WebDAV 目录创建请求。

### Refactor

- 拆分云同步设置、日期选择器、导入导出、保险库目录与恢复手动输入逻辑；共享复制反馈和增量分页 hook，保持既有行为。
- 收敛附件引用扫描、重加密单列处理、生物识别审计、插件事件分发及对象行映射；拆分附件导入、下载校验和恢复编排，清理未使用导出、变量与导入。

### Chore

- **开发版云同步基础设施** — 增加 WebDAV 配置、连接测试、加密快照上行/下行、自动导入、调度与 GFS 保留策略，支持移动端仅 Wi-Fi、索引条件更新与冲突重试、失败临时文件清理及 XML 协议解析。正式发布版隐藏入口并拒绝直达路由，此功能尚未对外开放。
- 插件市场子模块更新至包含 cloud-packager 1.0.0 的版本；补充桌面材质、Android 方案与实现、云同步实施及代码审查记录。
- 补充启动、桌面材质、尺标、侧栏、Android 导航/触控/附件、会话隔离及签名保密回归；生物识别与插件集成测试使用隔离临时数据，避免依赖 HOME 或用户真实账户（P050/P051）。
- 修复 Windows Python 检查入口、Rust 格式/Clippy 问题与测试超时预算；签名模型注册表保持原始换行，避免 Windows 检出改变签名内容。
...
- **编辑对象页同名字段推荐（一键填入）** — 编辑对象时，若其他对象存在同名字段且有内容，字段下方展示推荐条目，可一键填入；中文名与英文名视为同名字段，支持跨语言匹配。
- **字段推荐按敏感度分级揭示** — public/internal 明文展示；sensitive 掩码、点击揭示；critical 掩码、点击需验证主密码（支持密码/PIN/生物识别），验证后 1 分钟宽限期内再次查看/填入无需重复验证；揭示中的条目显示自动隐藏倒计时（每秒递减），到期自动重掩。
- **字段推荐超过 3 条折叠展示** — 底部提供展开/收起按钮查看其余条目。
- **日期/时间字段分段直输 + 日历双模式** — 支持年月日/时分秒分段直接输入；日期选择改为点击整个输入区域弹出日历。
- **对象详情与历史快照揭示倒计时** — sensitive/critical 字段查看后显示 1 分钟自动隐藏倒计时，到期自动回到遮掩状态；历史快照卡片对象名称样式增强，更清晰可读。

### Fixed

- **OneDrive 目录上传被白名单拒绝（N010）** — 文件系统白名单纳入 `~/OneDrive` 与 `OneDrive - <组织名>` 前缀目录，Windows 用户默认存放位置不再上传失败。
- **fs_get_file_size 失败静默记 0** — 白名单外路径上传不再静默显示 0B，直接提示上传失败。
- **附件操作失败提示本地化** — 上传/删除/转发/恢复/永久删除失败的 toast 中，后端英文错误详情（如 `Path is outside the allowed directory`）统一经 i18n 映射本地化。
- **照片集全屏查看器导航（T014）** — 全屏查看器的返回/关闭按钮直接退到相册上一层，不再逐级回退。

### Chore

- 版本号同步 2.12.2（package.json / tauri.conf.json / Cargo.toml / tauri.properties versionCode 2012002）。

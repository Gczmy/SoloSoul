...
- **模板动态字段组支持** — 模板新增动态字段组类型，支持对象中按需增删子字段、类型图标与敏感度徽章。
- **模板更新后对象手动同步** — 模板变更后在对象详情页提示同步，支持查看差异、一键同步或跳过。

### Fixed

- **模板同步一致性** — 修复前后端模板指纹不一致导致同步提示条反复显示的问题。
- **回收站预览与恢复** — 回收站内容预览正确显示字段值、动态字段组子字段树状展开，恢复对象时兼容旧版数据异常。
- **历史记录与审计日志** — 动态字段组相关记录正确国际化并显示具体子字段名。

### Changed

- **动态字段组作为模板级开关** — 统一前后端数据模型与 UI 渲染。
- **UI 细节优化** — 数据管理页面悬停动画、密码验证卡片置顶、页面切换 loading 闪烁修复。

### Chores

- 代码质量审计：移除冗余 clone、生产 unwrap 替换、前端与测试死代码清理。
- 版本号同步升级到 2.5.12。

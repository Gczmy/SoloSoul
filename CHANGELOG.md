...
- **同步冲突对话框布局重构：列表固定可见，详情区独立滚动** — 根因：关闭「只看差异」时 diff 表格显示全部字段行，内容超长把顶部冲突列表挤出 Dialog 视口（用户开启开关后内容骤减、滚动位置回弹到顶部，才看到列表，产生「开启能看到卡片、关闭看不到」的错觉）。修复：容器改为固定高度（`min(70vh, 560px)`），冲突列表固定在顶部（自身可滚动），详情区 `flex:1` 独立滚动——无论开关状态、无论字段多少，6 条冲突列表始终可见。

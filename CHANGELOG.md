...
- **同步冲突对话框底部操作按钮悬浮固定** — 「保留本地/保留远程/忽略」按钮栏改为 `position: sticky; bottom: 0`：Dialog 内滚动时始终悬浮贴底可见，用户无需滚到底即可快速处理冲突；左右负外边距铺满 Dialog 宽度、实底背景 + 顶边框遮挡下方内容透出，滚动到底自然到位不遮挡末行（浏览器三状态实测）。

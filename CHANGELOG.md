...
- **同步冲突对话框布局重构：列表 sticky 固定 + 对话框整体滚动** — 根因：关闭「只看差异」时 diff 表格显示全部字段行，内容超长把顶部冲突列表挤出 Dialog 视口（用户开启开关后内容骤减、滚动位置回弹到顶部，才看到列表，产生「开启能看到卡片、关闭看不到」的错觉）。第一版修复（固定高度容器 + 详情区独立滚动）被用户反馈「表格看不到、没有外部滚动条」——内部嵌套滚动区在 macOS 遮罩滚动条下不可感知。最终方案：容器不再固定高度/裁剪，**冲突列表 `position: sticky` 固定在对话框顶部**（负外边距铺满 Dialog 内边距 + 实底背景防透出），详情区自然伸展，整窗由 Dialog 外部滚动条统一滚动——列表始终可见、表格可滚动到底看到操作按钮。
...
- **冲突对话框表格不可见修复** — 上一版固定高度布局（`.container` `height: min(70vh,560px)` + `overflow: hidden`，详情区 `flex:1` 独立滚动）在用户实际环境出现「列表可见但表格看不到、无外部滚动条」：嵌套滚动区 + 遮罩滚动条导致详情无法感知滚动。改为列表 sticky 固定 + Dialog 整窗滚动方案（见上方 Changed），消除嵌套滚动区，恢复外部滚动条。

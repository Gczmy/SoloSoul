...
String _getFieldDisplayName(BuildContext context, String fieldId) {
  ...
  // 对复合路径加上 section 前缀（如 address.street → 地址.街道），
  // 帮助用户识别字段所属的数据类别。
  if (fieldId.contains('.')) {
    final sectionKey = fieldId.split('.').first;
    // 优先从 SectionRendererRegistry 获取 section 标签（与页面/Sidebar 一致），
    // 无 preset 的 section 再 fallback 到 ARB 通用翻译。
    final sectionLabel = SectionRendererRegistry.getSectionLabelByFieldPrefix(
          sectionKey, l10n,
        ) ??
        translateFieldLabel(sectionKey, l10n);
    // 仅当 section 有有效翻译（非机械格式化兜底）且与字段标签不同时加前缀
    if (sectionLabel != formatFieldLabel(sectionKey) && sectionLabel != label) {

...
pub fn load_models_from_memory_v2(
    ...
    if !det_bytes.is_empty() && DET_SESSION.get().is_none() {
        let session = ort::session::Session::builder()
            .map_err(|e| OcrError::InferenceFailed(format!("DET session builder failed: {e}")))?
            .commit_from_memory(det_bytes)
            .map_err(|e| OcrError::InferenceFailed(format!("DET model load failed: {e}")))?;
        DET_SESSION
            .set(Mutex::new(session))
            .map_err(|_| OcrError::InferenceFailed("DET session already initialized".to_string()))?;
    }

    if !cls_bytes.is_empty() && CLS_SESSION.get().is_none() {
        let session = ort::session::Session::builder()
            .map_err(|e| OcrError::InferenceFailed(format!("CLS session builder failed: {e}")))?
            .commit_from_memory(cls_bytes)
            .map_err(|e| OcrError::InferenceFailed(format!("CLS model load failed: {e}")))?;
        CLS_SESSION
            .set(Mutex::new(session))
            .map_err(|_| OcrError::InferenceFailed("CLS session already initialized".to_string()))?;
    }

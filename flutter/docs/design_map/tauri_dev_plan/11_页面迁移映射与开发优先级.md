...
>
> ⚠️ **术语迁移（审批通过）**：UI 中 "分区" → "集合"，"字段" → "属性"，`UnifiedObject` → `Object`。
> 详见文档 17 的术语规范。
...
| `AppShell` | 应用外壳（SideNavigation + AppBar + Content） |
...
所有页面必须在无网络下 100% 可用：
- ✅ 数据全部来自本地 Rust 后端（SQLite/文件系统）
- ✅ 无网络请求的加载态（没有 "Loading..." 因为等待网络）
- ✅ 同步相关页面在无网络时显示"未连接到任何设备"而非报错

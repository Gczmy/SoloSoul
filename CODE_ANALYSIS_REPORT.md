...
| P043 | P2 | 架构 | `tauri/src/stores/objectStore.ts:185-196` | `deleteObject` 不清 `currentObjectCache` 详情缓存，残留旧数据 | `[x]` 已修复（删除成功路径同步剔除 `currentObjectCache[objectId]`，防残留被误读） |
| P044 | P2 | 可维护性 | `hooks/useRevealState.ts:90-93`、`pages/auth/LoginPage.tsx:32` | 遗留注释：字段类型感知部分掩码规格未实现；`__DEBUG_SHOW_ALL` 调试开关遗留 | `[x]` 已修复（两处 `__DEBUG_SHOW_ALL`/`__DEBUG_SHOW_ALL_ICONS` 调试开关移除并简化 7 处条件；useRevealState 过时注释收拢为一行 TODO） |

...
> **关于重装后登录方式的设计决策**：详情见第 14 章「重装后登录方式分析：生物识别/PIN 与强制密码」。
...
## 14. 重装后登录方式分析：生物识别/PIN 与强制密码

### 14.1 背景

当用户卸载 SoloSoul 后重新安装，选择已有账户数据的外部 SAF 目录时，Onboarding 能够检测到已有账户并显示「登录已有账户」决策卡片。此时，用户可能曾经设置过指纹/人脸/PIN 等快捷解锁方式。

**核心问题**：是允许用户直接通过指纹/人脸/PIN 登录已有账户，还是强制要求用户输入原始主密码？

### 14.2 约束：零知识模型的密钥派生链

SoloSoul 的零知识安全模型决定了密钥派生链：

```
主密码 → Argon2id（salt + 迭代） → AES-256-GCM 加密密钥
         ↑ 从不存储，用完安全擦除（Zeroizing）
```

- **指纹/人脸**：绑定的不是主密码，而是 Android KeyStore 中存储的**加密会话密钥**（主密码的派生结果）。KeyStore 条目在签名一致时*可能*在卸载后存活，但本地元数据（哪个 KeyStore 别名对应哪个账户）已被卸载清除。
- **PIN**：存储在应用私有数据目录中，卸载后即被擦除。

### 14.3 方案对比

| 维度 | A：允许生物识别直接登录 | B：强制输入主密码（推荐） |
|------|------------------------|--------------------------|
| **安全性** | ❌ 卸载不再是一道安全边界；KeyStore 密钥是否存活取决于 Android 版本/实现，不可靠 | ✅ 零知识模型不受损害，主密码是唯一信源 |
| **可行性** | ❌ 需要扫描 KeyStore + 启发式匹配账户（脆弱、平台依赖、维护成本高） | ✅ 直接调用 `unlock`，流程确定 |
| **PIN 可用性** | ❌ 私有数据已擦除，PIN 肯定不可用 | ✅ 不依赖 PIN |
| **用户预期** | ❌ 用户安装安全应用时预期需要重新输入密码 | ✅ 符合行业惯例 |
| **实现复杂度** | 高（需要 KeyStore 扫描 + heuristic 匹配 + 多 ROM 适配） | 低（已有完整的密码登录链路） |

### 14.4 行业做法

| 应用 | 重装后 | 说明 |
|------|--------|------|
| **1Password** | 必须输入主密码 | 指纹/面容是会话便利，恢复后重新启用 |
| **Bitwarden** | 必须输入主密码 | 同上 |
| **ProtonMail** | 必须输入邮箱密码 | 生物识别是设备级快捷方式 |
| **Authy** | 必须走账号恢复流程 | 生物识别仅对已登录会话有效 |

行业共识很明确：**生物识别是会话层便利，不是账户恢复方法。**
...
1. **零知识模型保证**：主密码是唯一的密钥派生源，不允许绕过。
2. **卸载 = 安全边界**：用户卸载后再安装，应当预期需要原始密码。这与「重装银行 App 需要重新登录」「重装 1Password 需要主密码」的用户心智一致。
3. **PIN 不可用**：PIN 存储在应用私有数据，卸载后已永久消失。
4. **KeyStore 不可靠**：KeyStore 条目在卸载后是否存活取决于 Android 版本、制造商实现、签名密钥变更等，不能作为可靠恢复手段。

### 14.6 UX 补偿策略

为了缓解「明明设了指纹怎么还要输密码」的认知摩擦，建议在登录成功后实施以下补偿措施：

1. **检测之前启用的解锁方式**：在外部目录的账户 config.json 中，写入 `biometric_enabled` / `pin_enabled` 元数据（在用户首次启用生物识别/PIN 时同步保存到 SAF 目录，而非仅存于应用私有数据）。

2. **登录成功后引导重设**：
   - 登录成功后，前端检测 config.json 中的元数据。
   - 若检测到之前启用过生物识别，**立即弹出设置生物识别的引导卡片**：
     > 「检测到您之前使用过指纹解锁，是否重新设置？」
     > [是，设置指纹] [暂不]
   - 用户点击「是」→ 直接调用生物识别绑定流程。
   - 用户点击「暂不」→ 下次启动再次提示，最多提示 3 次。

3. **PIN 恢复策略**：
   - PIN 基于主密码派生，用户可以登录后重新设置 PIN。
   - 引导卡片同时提供：「是否设置 PIN 码快捷解锁？」。

### 14.7 实现路径

| 步骤 | 描述 | 优先级 |
|------|------|--------|
| 1. config.json 增加元数据字段 | `biometric_enabled: boolean`、`pin_enabled: boolean`、`has_biometric_history: boolean`（重装后通过 SAF 同步识别） | P1 |
| 2. 登录成功后前端检测 | 读取 `AccountSummary` 中新增的元数据字段，决定是否弹出引导卡片 | P2 |
| 3. 引导卡片 UI | 生物识别/PIN 设置引导卡片，支持「是，设置」「暂不」交互 | P2 |
| 4. 重装后检测不超过 3 次 | 本地 SharedPreferences 记录剩余提示次数 | P3 |

### 14.8 不做的事

- ❌ 不尝试绕过主密码直接使用生物识别登录。
- ❌ 不尝试恢复 PIN（PIN 存储在私有数据，卸载后无法恢复）。
- ❌ 不在 onboarding 阶段提示「您的设备支持指纹，是否跳过密码？」（这会削弱安全感知）。
- ❌ 不触发原生 KeyStore 扫描来「猜测」之前绑定的密钥。

> 本分析于 2026-07-25 评审确认，方案 B（强制密码登录 + 登录后引导重设）为最终决策。

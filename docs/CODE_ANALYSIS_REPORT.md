...
> 最后更新：2026-09-17（轮次 6：已完成 P037–P050，复核新增 P051 待修复；历史 P012 延期）
...
| P051 | P2 | 测试/数据隔离 | `src-tauri/tests/plugin_install.rs`、`plugin_sandbox.rs` | 集成测试使用默认用户插件目录，安装测试会卸载并重装真实目录中的官方插件 | `[ ]` 待修复 |
...
- 已关闭：49 / 51（含已修复及已确认设计例外；本轮完成 P037–P050）
- 未关闭：2 项（P012、P051）
- 当前处理：P051。原有子模块与生成资源改动保留；本轮修复按明确路径独立提交。
...
- **验证**：Store 与 IPC 15 文件 / 203 项回归通过；最终边界补充后，会话及 IPC 2 文件 / 39 项通过。覆盖 7 类缓存的迟到成功/失败、A→锁定→B、旧账户新调用不干扰 B、列表乱序、保存失败回滚、统计重置、模板后续刷新、插件重跑/跨账户、同步迟到监听及排队。TypeScript 和全量 ESLint 通过；全量 Vitest 正在收尾。
...
- **修复要求**：两处测试显式注入各自的 TempDir，市场资源保持只读；验证安装、卸载及测试目录清理，并单独提交。

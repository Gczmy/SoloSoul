...
> 最后更新：2026-09-17（轮次 6：已完成 P037–P051；历史 P012 延期）
...
| P038 | P1 | 安全/数据丢失 | `crates/solosoul-plugin/src/store.rs:17` | 插件 ID 校验允许 `.`/`..`，卸载时递归删除越过插件目录边界 | `[x]` 已修复 |
| P039 | P1 | 安全/会话隔离 | `src/stores/`、`src/App/AppRoutes.tsx` | 会话与请求代次隔离缓存、失败回滚、后续刷新及插件运行事件 | `[x]` 已修复 |
| P040 | P1 | 安全/存储 | `crates/solosoul-vault/src/storage/sync_apply.rs:142` | 同步冲突的双方完整数据明文写入普通 SQLite，绕过 Vault 应用层加密 | `[x]` 已修复 |
...
| P049 | P2 | 移动端导航 | `src/hooks/useOverlayBackGuard.ts:24` | 用对象引用识别 History 标记，真实浏览器结构化克隆后会把活跃相册当残留额外后退 | `[x]` 已修复 |
| P050 | P2 | 测试/跨平台 | `crates/solosoul-core/src/biometric/mod.rs` | 可用性测试依赖进程 HOME，Windows 缺失时 panic，且可能与其他测试环境修改竞态 | `[x]` 已修复 |
...
- 已关闭：50 / 51（含已修复及已确认设计例外；本轮完成 P037–P051）
- 未关闭：1 项（P012，沿用延期决定）
- 当前处理：无。本次完成 P038–P049 与复核新增 P050/P051，共 14 项，分别本地提交，未推送。P037 已在此前交付。原有子模块、Cargo.toml、三个 NSIS 位图与搜索索引改动保留。
- 最终验证：前端 120 文件 / 1014 项、GUI 459、共享核心 206、存储 180、插件 70、同步 64、密码学 23 项通过；类型、Lint、fmt、严格 Clippy 及 Markdown/ACL/偏好键检查通过。GUI 测试清单兼容处理、集成测试自行跳过的分支及未验证平台见终审报告，不声称原生跨平台或真实云服务已验收。

## 第六轮分析阶段记录（2026-09-17，以下保留原审查环境与授权状态）
...
- **修复说明（2026-09-17）**：认证状态同步订阅驱动会话代次及缓存清理；8 个 Store 在清理时废弃旧请求，列表/详情/设置键按请求序号拒绝乱序，await 恢复点检查代次，失败不得回滚新账户或触发后续刷新。插件保留公开目录、清除运行态，以运行代次隔离事件/结果，停止和清输出同样失效旧回调。同步监听注册/事件及 enable 排队任务受会话约束；清除同步事件合并缓存。AppRoutes 设置加载链检查原会话，账户变更重新挂载受保护壳层。IPC 真正发送前复核调用方 guard。
- **验证**：Store 与 IPC 15 文件 / 203 项回归通过；最终边界补充后，会话及 IPC 2 文件 / 39 项通过。覆盖 7 类缓存的迟到成功/失败、A→锁定→B、旧账户新调用不干扰 B、列表乱序、保存失败回滚、统计重置、模板后续刷新、插件重跑/跨账户、同步迟到监听及排队。TypeScript、全量 ESLint 和全量 Vitest（120 文件 / 1014 项）通过。
...
- **修复说明（2026-09-17）**：冲突双方载荷使用 Vault 密钥加密，详情解密后交给解决逻辑；锁定时拒绝敏感访问。独立迁移标记覆盖原有 v1 数据库，事务内校验及转换全部冲突，随后 VACUUM 与截断 WAL 成功才记录完成；失败可幂等重试。启用 secure_delete 减少后续删除残留；既存备份、快照和底层介质不在清理承诺内。冲突同时加入整库原子换钥和仅有冲突时的密钥探测。
- **验证**：存储单元测试 176 项通过，覆盖原始 SQL 密文、upsert、详情/解决、锁定拒绝、v1 旧库迁移及重复打开、当前数据库/WAL 字节无测试明文、损坏行迁移回滚和换钥失败不改其他表；严格 Clippy 通过。
...
- **修复说明（2026-09-17）**：聊天表加入整库换钥事务，改密和 KDF 升级共用此入口；只读密钥探测覆盖全部加密域，并跳过旧明文/空行及旧版尚不存在的表。无密文保持原有空库语义。
- **验证**：存储 178 项、共享核心 205 项单元测试通过，新增聊天独立探测、锁定重开和损坏行全事务回滚；真实账户改密及 KDF 升级后聊天仍可读取。历史损坏数据需旧密码/盐值另行恢复。
...
- **修复说明（2026-09-17）**：单字段与列对象接口共用授权投影；空权限、未声明类型/契约拒绝，名称单独授权，嵌套字段不带出同级值。typed 通配只覆盖已绑定字段，角色输出映射到用户实际字段；同契约不同模板按模板 ID 隔离，旧对象归属不唯一时拒绝猜测。仅声明 roles 的插件保持可用，结构树输出角色元数据，授权提示采用候选模板中最严格敏感度。
- **验证**：插件测试 63 项通过、2 原有忽略，严格 Clippy 和格式检查通过。新回归覆盖 street/secret、空权限、其他模板、嵌套对象/数组、通配、角色映射，以及到期提醒契约绑定在另一模板契约的兼容路径。未改动插件市场子模块。
...
- **修复说明（2026-09-17）**：随机运行目录在 Unix 创建时即为 0700，由阻塞 worker 持有 TempDir；成功、错误、panic、外层取消均按实际 worker 生命周期清理。附件子目录使用随机 ID，不将外部 ID 拼入临时路径。明文/解密复制共用创建时 0600 的 NamedTempFile，完整成功才原子替换目标，失败不残留部分明文且保留原目标。Windows 目录沿用用户 TEMP 的 ACL。
- **验证**：插件 70 项通过、2 原有忽略，共享核心 206 项通过；含测试的严格 Clippy 通过。覆盖 worker 成功/错误/panic/开始后取消，后段密文损坏、原目标保留、临时残留检查及旧明文复制。新增 Unix 0700/0600 检查在本机 Windows 不执行，需由 macOS/Linux CI 验证；不承诺清理旧版本遗留目录或进程强制终止后的残留。
...
- **P049 修复说明（2026-09-17）**：浮层与内层各生成可序列化唯一标记 ID，sweeper、所属浮层判断及卸载微任务均比较 ID；其他活跃浮层的标记不会被旧浮层卸载清掉。测试模拟每次读取 History state 都结构化克隆。
- **P049 验证**：11 项 hook 回归通过，覆盖克隆所有权、分层返回、残留标记、外部登录导航、StrictMode 和另一浮层。TypeScript、定向 ESLint 通过；相册首次 12 项中 11 项通过，首项在懒加载 fallback 超时，与初始基线相同。后台链接结束后单独复跑 12 项全通过，最终全量 Vitest 120 文件 / 1014 项通过；尚未 Android 真机验证。

### P050 — 生物识别可用性测试依赖 HOME（P2，修复复核新增）

- **证据**：Windows 全工作区构建后执行 core 206 项，205 通过，`test_macos_biometric_availability_shape` 在 `manager_from_home` 的 `var("HOME").unwrap()` 失败；补齐 CARGO_MANIFEST_DIR 仍失败。该项未通过专用临时目录初始化，还可能偶然借用并行测试临时设置的 HOME。
- **修复**：测试直接使用 TempDir 和测试存储构造 manager，不依赖或覆盖真实 HOME，也不访问真实 Vault。
- **验证**：`cargo fmt --check`、core 含测试目标的严格 Clippy（启用 ocr/pdf/watermark）通过；同特性核心 206 项测试全部通过。

### P051 — 插件集成测试使用默认用户数据目录（P2，修复复核新增）

- **证据**：安装测试使用 `PluginManager::new()`，先卸载再安装 `com.solosoul.official.phone-fmt`；该构造函数的桌面数据目录为真实用户主目录中的 `.solosoul`，没有测试隔离。沙箱构造测试也使用默认目录，会加载现有插件审计记录。
- **本次影响**：此前全量回归已执行该安装测试。只读文件元数据确认默认目录中的该插件目录和 `plugin_audit.jsonl` 在测试运行时更新；没有运行前插件状态可供可靠恢复，因此不猜测删除或回滚。该发现已告知用户。
- **修复**：两处测试显式注入各自的 TempDir，市场资源保持只读；安装测试断言 manifest/Wasm 确实写入临时目录，并验证卸载。两处均在释放 manager 后显式关闭临时目录，检查清理结果。
- **验证**：fmt、两个集成目标的严格 Clippy 通过；安装/卸载、独立初始化和陷阱隔离共 3 项实际执行通过，另 1 项 hello-world 因示例 Wasm 缺失自行跳过。比较测试前后的原用户插件目录和审计日志元数据，无变化。无用临时备份、脚本、日志及 GUI 测试程序副本清理后交付。

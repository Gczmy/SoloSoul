...
> 最后更新：2026-09-17（轮次 6：按用户要求逐项修复，已完成 P037、P038、P040、P041、P042、P043、P045、P047、P048、P044、P046、P049、P039）
...
| P050 | P2 | 测试/跨平台 | `tauri/crates/solosoul-core/src/biometric/mod.rs` | 可用性测试依赖进程 HOME，Windows 缺失时 panic，且可能与其他测试环境修改竞态 | `[ ]` 待修复 |

## 修复进度

- 已关闭：48 / 50（含已修复及已确认设计例外；本轮完成 P037、P038、P040、P041、P042、P043、P045、P047、P048、P044、P046、P049、P039）
- 未关闭：2 项（P012、P050）
- 当前处理：P050。原有子模块与生成资源改动保留；本轮修复按明确路径独立提交。
...
- **P049 验证**：11 项 hook 回归通过，覆盖克隆所有权、分层返回、残留标记、外部登录导航、StrictMode 和另一浮层。TypeScript、定向 ESLint 通过；相册 12 项中 11 项通过，首项仍在懒加载 fallback 超时，与初始基线相同，待后台链接结束后单独复核；尚未 Android 真机验证。
...
- **修复要求**：测试直接使用 TempDir 和测试存储构造 manager，不依赖或覆盖真实 HOME，也不访问真实 Vault；单独提交并复跑核心测试。

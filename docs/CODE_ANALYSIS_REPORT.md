...
| A-002 | P2     | 安全·内存卫生  | `tauri/src/App/AppRoutes.tsx:248-263`                                  | syncStore（设备元数据：地址/指纹）、llmStatsStore（账号用量统计）未在 vault-locked 时清理          | `[x]` 已修复 |
...
- `syncStore.ts`：新增 `clearOnVaultLock` action，清空 `localFingerprint`/`connectedPeers`/`lastResult`/`recentResults`/`discoveredDevices`/`listenAddr`/`conflicts`/`selectedConflict`/`hasUnreadConflicts`/`error` 及配对中状态；保留纯 UI 开关类字段。
- `AppRoutes.tsx`：`vault-locked` 监听器中追加调用 `useSyncStore.getState().clearOnVaultLock()` 与 `useLlmStatsStore.getState().clear()`（复用其已有 `clear()`）。

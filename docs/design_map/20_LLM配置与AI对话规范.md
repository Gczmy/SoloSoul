...
> **前置阅读**：`13_用户数据边界与加密存储.md`、`24_矛盾冲突与决策记录.md`、`10_跨平台视觉规范与主题系统.md`（侧边栏规范）、`archive/Flutter_LLM_实现分析与Tauri借鉴建议.md`（已归档）
...
| archive/Flutter_LLM_实现分析 | 系统提示词模板、上下文注入机制、帮助文档检索的 Flutter 参考实现 |
...
> **历史**：本节内容源自 `archive/28_Tauri_LLM_模式B与Phase2实施计划.md`（已并入本文档并归档至 `archive/`）。原文包含逐阶段实施细节、代码草图与验收标准，需要回溯历史细节可查阅归档副本。

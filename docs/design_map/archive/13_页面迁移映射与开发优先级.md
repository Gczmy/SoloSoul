# 13 — 页面迁移映射与开发优先级
> **⚠️ 已归档（2026-08-07）**：Flutter→React 页面迁移**已全部完成**（实际 20 个页面）。本文档为迁移期的历史映射表，保留作追溯参考。当前页面结构与路由见 `10_前端技术架构与组件映射.md`。

> **前置阅读**：`10_前端技术架构与组件映射.md`、`12_状态管理_Zustand_Store设计.md`
> **Manifesto 对齐**：本地优先（所有页面离线可用）
> **源文档**：`tauri_refactor/页面迁移映射.md`
>
> **[状态] 已完成（2026-08）**：Flutter → React 迁移已全部完成，当前应用为纯 React + Tauri 实现。
> 本文档保留作为页面路由与优先级的历史映射记录，实际路由以 `src/App/` 为准。
>
...
| SecuritySettingsPage | 自动锁定时间选择，生物识别开关（注册/测试/禁用，详见文档 21），修改密码对话框 |
| SensitivitySettingsPage | 见文档 14（敏感度等级系统重构） |
...
| ExportImportPage | 见文档 17（导入导出功能设计） |
...
| 页面 | 关键验收标准 |
|------|-------------|
| LlmChatPage | 左右布局（对话侧边栏+消息区）、流式响应、消息气泡带时间戳和复制按钮、输入框上方模型信息栏（提供商·模型·类型标签）、对话自动命名/手动重命名、按最后更新时间倒序排列、右上角设置入口跳转 LLM 配置、AI 未开启/未配置/无网络引导态、用户授权对象数据查询 |
| PluginDashboardPage | 见文档 19（插件系统迁移设计） |
| SyncPage | 见文档 18（同步功能重构路线图） |
...
| `AppShell` | 应用外壳（SideNavigation 上下分区 + AppBar + Content），详见文档 11 第 9 节 |

...
## 单行顶部栏与统一拖拽高度（2026-09-15，首版；尺寸已在后文调整）

首轮保留了“原生标题栏避让 + 56px AppBar”的纵向叠加，本机总高度为 88px；两块区域都能拖动，造成正文空间浪费，左右拖拽高度也不一致。后续将标题、返回按钮和页面操作移入同一条原生标题栏：

- macOS AppBar 从窗口 y=0 开始，高度直接复用原生测量值，本机为 32px。全屏隐藏原生标题栏后仍保留 32px 紧凑页面操作行。正文保留 16px 内边距，删除原先额外的 56px 占位；尺标的顶部边界同步更新。
- `set_titlebar_color` 另返回三个系统按钮实际可见区域的右边界。左侧导航折叠以及右侧、顶部、底部导航时，标题和返回按钮使用横向安全距离避让交通灯，不增加栏高。顶部导航在 macOS 排在 AppBar 下方，其自身不再拖拽；其他平台沿用既有布局。
- 整窗仅原生标题栏空白区域拖拽，macOS 的网页 AppBar 和横向导航取消额外拖拽标记。原生 `NSView.hitTest:` 根据网页实时上报的控件矩形将按钮事件交给 WKWebView；路由变化、控件尺寸变化、侧栏折叠及窗口缩放时重新测量，页面卸载时清空。仍由原生标题栏处理后台首次拖拽与双击，不改变所有网页控件的首次点击策略。
- 交通灯原生位置、完整 WebView、玻璃视图和 alpha=0.001 底色均沿用前轮方案。

原生回归新增控件命中与拖拽边界断言：顶部空白命中拖拽层，登记的按钮矩形命中 WebView，清空矩形后恢复空白拖拽，标题栏下方始终命中 WebView。既有主题、隐藏/显示、缩放、全屏及视图身份断言继续通过。浏览器回归检查单行高度、四种导航布局、交通灯避让、动态按钮矩形、快捷卡片和尺标。

验证结果：TypeScript、目标文件 ESLint、Rust Clippy（`-D warnings`）、Rust fmt 和 ACL 一致性检查通过；原生窗口回归、8 项前端单元测试通过。标题栏及尺标 15 个浏览器场景在 Chromium、WebKit 均通过；既有 6 项桌面导航回归也在两个引擎通过。缩小顶部栏后，尺标可用高度增加，相关测试的旧固定高度预期已更新。

自动化仍分别验证原生命中与模拟 IPC 的网页布局，不能替代真实客户端的手动拖拽体验验收。

## 增高顶部栏与交通灯安全宽度（2026-09-15）

根据后续视觉反馈，32px 栏高和 48px 折叠侧栏过于紧凑。顶部栏改用原生 `NSToolbar` 的 `Unified` 样式，并使用 `IconOnly` 避免额外的图标标签行：本机离屏测量高度为 52pt，交通灯中心距窗口顶边 26pt，由 AppKit 自动居中。AppBar 与原生拖拽区继续读取同一个实测高度；标题增至 18px，顶部操作文字为 14px。系统定义见 [NSWindowToolbarStyleUnified](https://developer.apple.com/documentation/appkit/nswindow/toolbarstyle-swift.enum/unified?language=objc)。旧系统通过 selector 可用性检查沿用默认工具栏，不调用 macOS 11+ 才提供的方法。

macOS 折叠侧栏宽度改为 `max(96, ceil(trafficLightsRight + 16))`，将整个交通灯按钮组及右侧留白纳入同一背景。侧栏本体、AppBar、正文、尺标及快捷卡片统一读取 `--sidebar-width`；标准按钮在全屏隐藏时仍保留至少 96px，Windows 的折叠宽度继续为 48px。

原生回归从窗口最外层验证标题栏空白、网页控件、交通灯按钮的命中，检查 52px 范围内的拖拽边界、交通灯垂直居中及全屏往返。全屏下 AppKit 可以保留工具栏，不能继续要求测量高度必定为 0；回归改为验证不叠加额外高度，退出后恢复普通窗口栏高。浏览器回归新增按钮字号、折叠边界与交通灯留白、较宽交通灯范围、折叠后的卡片位置及全屏宽度稳定性。

本轮验证：Chromium / WebKit 的 22 项桌面导航、标题栏与尺标场景通过，最终操作字号调整后再次通过两个引擎的 10 项标题栏回归。8 项前端单元测试、TypeScript、目标文件 ESLint、Rust Clippy（`-D warnings`）、Rust fmt 通过。原生回归通过，实测普通窗口和全屏均为 52pt，交通灯居中，标题栏下方不拖拽，玻璃/WebView/按钮身份保持稳定。

...
2. [安装](#安装)
3. [快速开始](#快速开始)
4. [Flutter macOS 应用](#flutter-macos-应用)
5. [Web 界面 (维护模式)](#web-界面-维护模式)
6. [命令行工具](#命令行工具)
7. [档案管理](#档案管理)
8. [OCR 扫描](#ocr-扫描)
9. [Plugin 管理](#plugin-管理)
10. [安全说明](#安全说明)
11. [故障排除](#故障排除)
...
- **📱 Flutter 原生应用** - macOS 原生体验，高性能跨平台客户端
...
- **操作系统**: macOS 10.15+ (Flutter 应用)
- **内存**: 建议 4GB+ (Argon2 加密需要)
- **磁盘**: 至少 100MB 可用空间

### Flutter macOS 应用 (推荐)

下载并安装 SoloSoul DMG 安装包:

1. 下载 `SoloSoul-{version}-macos.dmg`
2. 双击打开 DMG 文件
3. 将 SoloSoul 应用拖入 Applications 文件夹
4. 首次运行时，在 Finder 中右键点击应用选择"打开"，允许来自未识别开发者的应用
...
1. 输入您的 **Master Password**
2. 点击 **Unlock** 解锁
3. Vault 解锁后即可访问所有功能

### 3. 创建和编辑档案

1. 在侧边栏选择档案类型 (**Profile / Travel / Financial / Professional**)
2. 点击 **Add** 或编辑现有档案
3. 填写信息后点击 **Save**

### 4. 使用 OCR 扫描

1. 选择文档类型 (Passport / National ID / Visa)
2. 上传或拖拽图片
3. 点击 **Scan** 自动提取信息
4. 确认后保存到档案
...
SoloSoul Flutter 应用提供原生 macOS 体验：
...
> ⚠️ **注意**: Web 界面已处于维护模式，不再 active 开发。
> 建议使用 Flutter macOS 应用获取最新功能和安全性更新。

                ...
                >
                  ...
                  >
                    {detailItem.previewProperties.map((p, i) => {
                      const propType = (p as Record<string, unknown>).type as
                        | PropertyType
                        | undefined;
              ...
              {detailItem.itemType !== 'template' && (
                <>
                  ...
                  >
                    <div
                      onClick={() =>
                        setExpandedSections((prev) => ({ ...prev, attachments: !prev.attachments }))
                      }
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    >
                      ...
                      {detailItem.attachments.length + detailItem.deletedAttachments.length})
                    </div>
                      ...
                      <div style={{ marginTop: 8 }}>
                        ...
                        {detailItem.deletedAttachments.length > 0 && (
                          <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                            ...
                            >
                              {t('common:active')} ({detailItem.attachments.length})
                            </button>
                            <button
                            ...
                            >
                              {t('common:trash')} ({detailItem.deletedAttachments.length})
                            </button>
                          </div>
                        )}
                        {(showTrashAttachments
                          ? detailItem.deletedAttachments
                  ...
                  >
                    <div
                      onClick={() =>
                        setExpandedSections((prev) => ({ ...prev, snapshots: !prev.snapshots }))
                      }
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                            ...
                            {detailItem.snapshots.length > 1 && (
                              <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                                <button
                                  ...
                                  onClick={() =>
                                    changeSnapshot(detailItem.id, detailItem.snapshots, snapIdx + 1)
                                  }
                                  style={{
                                    padding: '3px 8px',
                                    border: '1px solid var(--border-subtle)',
                                ...
                                <span style={{ padding: '3px 0', color: 'var(--text-tertiary)' }}>
                                  {snapIdx + 1} / {detailItem.snapshots.length}
                                </span>
                                <button
                                  disabled={snapIdx <= 0}
                                  onClick={() =>
                                ...
                                >
                                  <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                                    {currentSnap.diffSummary &&
                                      !(
                                        detailItem.snapshots.length > 2 &&
                                        snapIdx === detailItem.snapshots.length - 1 &&
                                        ...
                                        >
                                          {t(
                                            `common:diff_${currentSnap.diffSummary}`,
                                            currentSnap.diffSummary,
                                          )}
                                        </span>
                                      )}
                                    <SnapshotVersionBadge
                                      index={snapIdx}
                                      total={detailItem.snapshots.length}
                                    />
                                    <span
                                      style={{
                                        padding: '2px 6px',
                                    ...
                                    >
                                      {t(
                                        `common:trigger_${currentSnap.triggeredBy}`,
                                ...
                                <div style={{ minHeight: 60 }}>
                                  ...
                                  {data &&
                                    (() => {
                                      ...
                                      const rawProps = d.properties as
                                          ...
                                          if (
                                            ...
                                            orderedFields.push({
                                              ...
                                              sensitivityLevel: (p.sensitivityLevel ||
                                                'internal') as SensitivityLevel,
                                            });
                                          }
                                        }
                                        // Orphaned fields not in template
                                        const known = new Set(
                                          detailTemplate.properties.map((p) => p.id),
                                        );

...
function PieChartSvg({ slices, size }: { slices: PieSlice[]; size: number }) {
  ...
  const arcs = slices.map((slice) => {
    ...
    return <path key={slice.key} d={path} fill={slice.color} stroke="var(--bg-primary)" strokeWidth={1} />;
  });
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block' }}>
      {arcs}
      {total > 0 && (
        <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central" fontSize={13} fontWeight={600} fill="var(--text-primary)">
          {formatBytes(total)}
        </text>
      )}
    </svg>
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <Card>
          ...
          {stats && breakdownItems.length > 0 && (
            <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--border-subtle)', fontSize: 12, color: 'var(--text-tertiary)', display: 'flex', flexDirection: 'column', gap: 3 }}>
        ...
        {showBreakdown && stats && (
          <div
            ...
            style={{
              ...
              width: 340, maxHeight: '80vh', overflowY: 'auto', zIndex: 100,
              background: 'var(--bg-elevated)', borderRadius: 12, padding: 20,
              boxShadow: '0 8px 32px rgba(0,0,0,0.2)',
            }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ fontSize: 15, fontWeight: 600, margin: 0 }}>{t('settings:storage_breakdown')}</h3>
              <button
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {pieSlices.map((slice, idx) => {
                ...
                return (
                  <div key={slice.key} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
                    <div style={{ width: 12, height: 12, borderRadius: 3, background: slice.color, flexShrink: 0 }} />
              ...
              {/* Total */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, borderTop: '1px solid var(--border-subtle)', paddingTop: 8, marginTop: 4 }}>

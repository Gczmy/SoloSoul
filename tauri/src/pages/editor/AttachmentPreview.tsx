...
export function AttachmentPreview({ fileName, mimeType, filePath, onClose }: AttachmentPreviewProps) {
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {isImage && (
            <>
              ...
              <button onClick={() => setZoom(z => Math.min(400, z + 25))} style={btnStyle}>+</button>
            </>
          )}
          <button onClick={onClose} style={{ ...btnStyle, fontSize: 16 }}><X size={18} /></button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
        {isImage && filePath && (
          <img
            ...
            alt={fileName}
            style={{ maxWidth: `${zoom}%`, maxHeight: `${zoom}%`, objectFit: 'contain', borderRadius: 8 }}
          />
        )}
        {isPDF && filePath && (
          <iframe

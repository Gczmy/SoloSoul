...
/** 从当前 connectedPeers 解析对端信息（按 node id 或地址匹配），供同步历史盖章。
 *  记录时即固化名称/类型，避免对端之后被忘记导致历史条目无法追溯设备。 */
function findPeerInfo(target: string | null): { name?: string; clientType?: string } {
  if (!target) return {};
  const peer = useSyncStore
    .getState()
    .connectedPeers.find((p) => p.id === target || p.addr === target);
  if (!peer) return {};
  return { name: peer.name, clientType: peer.clientType };
}
...
const syncCompletedMergeCache = new Map<string, { lastAt: number; merged: SyncResult }>();
  ...
  return {
    ...
    syncWithDevice: async (deviceId) => {
      ...
      try {
        ...
        // 同步活动面板：普通失败（连接失败/对端离线等）写入失败历史条目，
        // 便于用户回溯哪台设备在何时同步失败（旧版失败仅横幅一闪而过、无从追溯）。
        // lastResult 保持 null（失败不触发「同步完成」toast），错误由页面横幅展示。
        const peer = findPeerInfo(deviceId);
    ...
    initSyncCompletedListener: (): Promise<UnlistenFn> => {
      return listen<{
        ...
        const isMerge = !!cached && now - cached.lastAt <= SYNC_COMPLETED_MERGE_WINDOW_MS;
        if (isMerge) {
        ...
        // 同步活动盖章：本地时间戳 + 对端设备信息（B 侧已持久化对端记录）
        const peer = findPeerInfo(p.peerNodeId);
        ...
        useUiStore.getState().showToast({
          type: 'success',
          message: i18next.t('settings:sync_completed_inbound', {
            examined: p.examined,
            applied: p.applied,
            skipped: p.skipped,
            outbound,

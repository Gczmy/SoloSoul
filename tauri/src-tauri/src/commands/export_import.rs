...
pub struct ExportScopeNode {
    pub id: String, pub name: String, pub node_type: String,
    pub children: Vec<ExportScopeNode>,
    pub item_count: usize, pub attachment_count: usize,
}

#[derive(Serialize)]
pub struct ExportEstimate {
    pub object_count: usize, pub estimated_bytes: u64,
}

#[derive(Deserialize)]
pub struct ExportRequest {
    pub profile_ids: Vec<String>,
    pub include_preferences: bool, pub include_audit_log: bool,
    pub password: String, pub save_path: String,
}
...
pub async fn export_get_scope_tree(
    ...
    Ok(profiles.iter().map(|p| ExportScopeNode {
        id: p.id.clone(), name: p.name.clone(), node_type: "profile".to_string(),
        children: vec![], item_count: 1, attachment_count: 0,
    }).collect())
}
...
pub async fn export_estimate_size(
    ...
    for id in &profile_ids {
        if let Ok(Some(p)) = vault.load_profile(id) { total += p.data.len() as u64; count += 1; }
    }
    Ok(ExportEstimate { object_count: count, estimated_bytes: total })
}
...
pub async fn export_execute(
    ...
    let zip_path = if req.save_path.ends_with(".solosoul") { req.save_path.clone() }
                   else { format!("{}.solosoul", req.save_path) };
...
#[tauri::command]
pub async fn import_preview_package(
    file_path: String,
    ...
    for i in 0..archive.len() {
        ...
        if name == "manifest.json" {
            entry.read_to_string(&mut manifest_str).map_err(|e| e.to_string())?;
            got_manifest = true;
            break;
        }
    }
    ...
    let export_time = manifest.get("export_time").and_then(|v| v.as_str()).map(|s| s.to_string());
    let profile_count = manifest.get("profile_count").and_then(|v| v.as_u64()).unwrap_or(0) as usize;
...
pub async fn import_execute(
    ...
    for i in 0..archive.len() {
        ...
        if name.starts_with("profiles/") && name.ends_with(".enc") {
            let profile_id = name.trim_start_matches("profiles/").trim_end_matches(".enc").to_string();
            let mut data = Vec::new();
            entry.read_to_end(&mut data).map_err(|e| e.to_string())?;

            // Don't overwrite existing profiles
            if vault.load_profile(&profile_id).map_err(|e| e.to_string())?.is_some() {
                continue;
            }

...
在两个实例的「设备同步」页面中打开「启用同步」，记录各自显示的 fingerprint。

### 3. 信任对端

- 若 mDNS 正常，未信任设备会出现在已知设备列表中。
- 点击「信任并配对」。
...
- 输入对端 `host:port`（如 `127.0.0.1:<port>`，port 在「设备同步」页面中不直接显示，可通过对端日志或 mDNS 获取），点击同步；或
- 直接点击已知设备行的同步按钮（如果 UI 已提供）。

### 5. 验证数据一致

在 device-a 中：

- 创建一个档案、一个对象、一个用户模板，并上传一个附件。
- 点击同步。

在 device-b 中：

- 等待同步完成，刷新对应页面，确认数据与附件均已到达。
- 在 device-a 删除该档案，再次同步，确认 device-b 中该档案也被删除（验证删除墓碑传播）。
...
- 检查对象的 `__attachments` 字段是否包含正确 `id`、`objectId`、`fileName`、`sizeBytes`。

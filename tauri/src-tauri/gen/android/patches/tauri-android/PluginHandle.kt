...
// SoloSoul PATCH（backport tauri upstream #15798，与 PluginManager.kt 配套）：
// PluginManager.activity 由 lateinit 改为可空（Activity 全部销毁时为 null），
// validatePermissions 增加空值守卫，其余逻辑不变。
...
class PluginHandle(private val manager: PluginManager, val name: String, val instance: Plugin, val config: String, val jsonMapper: ObjectMapper) {
  ...
  private fun validatePermissions(
    ...
    // SoloSoul PATCH: PluginManager.activity 可空（#15798），无 Activity 时直接拒绝
    val activity = manager.activity
      ?: run {
        invoke.reject("No activity available to validate permissions")
        return false
      }
    val prefs =

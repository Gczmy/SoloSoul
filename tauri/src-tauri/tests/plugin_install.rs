use solo_soul::plugin::{PluginManager, PluginStore};
...
fn test_install_plugin_from_market() {
    ...
    let info = entry.iter().find(|i| i.plugin_id == plugin_id).expect("找不到插件");
    let version = info.registry_entry.latest_version.clone().expect("无最新版本");

    println!("home: {:?}", dirs::home_dir());
    println!("plugin dir exists? {:?}", PathBuf::from("~/.solosoul/plugins").exists());

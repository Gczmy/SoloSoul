...
fn test_extract_attachment_ids_from_json_text() {
    ...
    assert!(
        VaultStore::extract_attachment_ids_from_json_text(r#"{"note":"__attachments"}"#).is_empty()
    );
    // 段内字符串含方括号（应被字符串状态跳过）
...
fn test_extract_attachment_ids_skips_escaped_quote_marker() {
    ...
    let text = r#"{"note":"escaped \"__attachments\": fake","__attachments":[{"id":"real_1"}]}"#;
    let ids = VaultStore::extract_attachment_ids_from_json_text(text);
    assert_eq!(
        ids,
...
fn test_extract_attachment_ids_recovers_after_unparseable_candidate() {
    ...
    // 修复后应继续向后搜索真实键。
    let text = r#"{"__attachments":[{"id":"broken"}},"other":1,"__attachments":[{"id":"ok_2"}]}"#;
    let ids = VaultStore::extract_attachment_ids_from_json_text(text);
...
fn test_property_type_infer_from_value() {
    ...
    assert_eq!(
        PropertyType::infer_from_value(&serde_json::json!("https://example.com"), "website_url"),
        PropertyType::Url
    );
    assert_eq!(
...
fn test_list_object_changes_paginated_ordering_and_completeness() {
    ...
    let paged_ids =
        collect_paginated_object_ids(&vault, watermark.clone(), "test_account", "local_node", 2);

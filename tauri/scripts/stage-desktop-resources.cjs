#!/usr/bin/env node
/**
 * 为桌面端构建生成精简后的插件市场资源目录。
 *
 * Tauri 的 bundle.resources 会原样把 `SoloSoul_plugin_market/plugins` 整个目录打进安装包，
 * 其中包含源码和构建配置（`lib.rs`、`Cargo.toml` 等），运行时只需要 `manifest.json` 和
 * `plugin.wasm`。该脚本只复制每个插件运行所需的最小文件：
 ...
 *
 * 输出目录 `src-tauri/resources-desktop/` 已加入 .gitignore，不会污染源码。
 */
...
const destDir = path.join(projectRoot, 'src-tauri', 'resources-desktop', 'SoloSoul_plugin_market');
...
function stagePluginMarket() {
  if (!fs.existsSync(srcDir)) {
    console.warn(`[stage-desktop-resources] 插件市场目录不存在，跳过: ${srcDir}`);
    return;
  }
  ...
  console.log(`[stage-desktop-resources] 已生成桌面端插件市场资源: ${destDir}`);
}
